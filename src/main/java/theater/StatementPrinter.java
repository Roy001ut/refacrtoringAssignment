...
public class StatementPrinter {
    ...
    public String statement() {
        ...
        for (Performance p : invoice.getPerformances()) {
            ...
            result.append(String.format("  %s: %s (%s seats)%n", getPlay(p).getName(),
                    frmt.format(getThisAmount(p, getPlay(p)) / Constants.PERCENT_FACTOR), p.getAudience()));
    ...
    private int getThisAmount(Performance performance, Play play) {
        ...
        switch (this.getPlay(performance).getType()) {
            case "tragedy":
                rlst = Constants.TRAGEDY_BASE_AMOUNT;
                if (performance.getAudience() > Constants.TRAGEDY_AUDIENCE_THRESHOLD) {
                    rlst += Constants.TRAGEDY_OVER_BASE_CAPACITY_PER_PERSON
                            * (performance.getAudience() - Constants.TRAGEDY_AUDIENCE_THRESHOLD);
                }
                break;
            case "comedy":
                rlst = Constants.COMEDY_BASE_AMOUNT;
                if (performance.getAudience() > Constants.COMEDY_AUDIENCE_THRESHOLD) {
                    rlst += Constants.COMEDY_OVER_BASE_CAPACITY_AMOUNT
                            + (Constants.COMEDY_OVER_BASE_CAPACITY_PER_PERSON
                            * (performance.getAudience() - Constants.COMEDY_AUDIENCE_THRESHOLD));
                }
                rlst += Constants.COMEDY_AMOUNT_PER_AUDIENCE * performance.getAudience();
                break;
            default:
